...
try {
const response = await fetch('http://localhost:3000/locks');
const data = await response.json();
setLocks(data);
} catch (error) {
...
try {
const response = await fetch(`http://localhost:3000/locks/${lockId}`, {
method: 'PUT',
headers: {
'Content-Type': 'application/json',
},
...
/>
<button onClick={() => addLock(newLockId)}>Add Lock</button> {/* You will implement this function as POST to add a lock */}
        ...
        {locks.map((lock) => (
          <tr key={lock.id}>
            ...
            <td>
              ...
              {/* Update this with the toggle function as needed */}
            </td>
          </tr>
        ))}
      </tbody>
